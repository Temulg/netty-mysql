...
public final class ErrorNumbers {
	...
	public static void dumpSqlStatesMappingsAsXml() throws Exception {
		Map<Object, String> ErrorNumbersToNames = new HashMap<>();
		ImmutableSortedSet<
			Integer
		> allErrorNumbers = ImmutableSortedSet.<Integer>naturalOrder().addAll(
			MYSQL_TO_SQL99_STATE.keySet()
		).build();

		for (Field possibleField: ErrorNumbers.class.getDeclaredFields()) {
			String fieldName = possibleField.getName();
			if (fieldName.startsWith("ER_")) {
				ErrorNumbersToNames.put(
					possibleField.get(null), fieldName
				);
			}
		}
		...
		allErrorNumbers.forEach(errorNumber -> {
			System.out.format(
				"   <ErrorMapping mysqlErrorNumber=\"%d\" "
				+ "mysqlErrorName=\"%s\" legacySqlState=\"\" "
				+ "sql92SqlState=\"%s\"/>\n",
				errorNumber,
				ErrorNumbersToNames.getOrDefault(
					errorNumber, ""
				), mysqlToSql99(errorNumber)
			);
		});
	...
	public static String get(String stateCode) {
		return SQL_STATE_MESSAGES.getOrDefault(stateCode, "");
	}

	private static String mysqlToSql99(int errno) {
		return MYSQL_TO_SQL99_STATE.getOrDefault(
			errno, SQL_STATE_CLI_SPECIFIC_CONDITION
		);
	}
	...
	private static final ImmutableMap<
		String, String
	> SQL_STATE_MESSAGES = ImmutableMap.<String, String>builder().put(
		SQL_STATE_DISCONNECT_ERROR, Messages.getString("SQLError.35")
	).put(
		SQL_STATE_DATE_TRUNCATED, Messages.getString("SQLError.36")
	).put(
		SQL_STATE_PRIVILEGE_NOT_REVOKED,
		Messages.getString("SQLError.37")
	).put(
		SQL_STATE_INVALID_CONNECTION_ATTRIBUTE,
		Messages.getString("SQLError.38")
	).put(
		SQL_STATE_ERROR_IN_ROW, Messages.getString("SQLError.39")
	).put(
		SQL_STATE_NO_ROWS_UPDATED_OR_DELETED,
		Messages.getString("SQLError.40")
	).put(
		SQL_STATE_MORE_THAN_ONE_ROW_UPDATED_OR_DELETED,
		Messages.getString("SQLError.41")
	).put(
		SQL_STATE_WRONG_NO_OF_PARAMETERS,
		Messages.getString("SQLError.42")
	).put(
		SQL_STATE_UNABLE_TO_CONNECT_TO_DATASOURCE,
		Messages.getString("SQLError.43")
	).put(
		SQL_STATE_CONNECTION_IN_USE, Messages.getString("SQLError.44")
	).put(
		SQL_STATE_CONNECTION_NOT_OPEN,
		Messages.getString("SQLError.45")
	).put(
		SQL_STATE_CONNECTION_REJECTED,
		Messages.getString("SQLError.46")
	).put(
		SQL_STATE_TRANSACTION_RESOLUTION_UNKNOWN,
		Messages.getString("SQLError.47")
	).put(
		SQL_STATE_COMMUNICATION_LINK_FAILURE,
		Messages.getString("SQLError.48")
	).put(
		SQL_STATE_INSERT_VALUE_LIST_NO_MATCH_COL_LIST,
		Messages.getString("SQLError.49")
	).put(
		SQL_STATE_NUMERIC_VALUE_OUT_OF_RANGE,
		Messages.getString("SQLError.50")
	).put(
		SQL_STATE_DATETIME_FIELD_OVERFLOW,
		Messages.getString("SQLError.51")
	).put(
		SQL_STATE_DIVISION_BY_ZERO, Messages.getString("SQLError.52")
	).put(
		SQL_STATE_ROLLBACK_SERIALIZATION_FAILURE,
		Messages.getString("SQLError.53")
	).put(
		SQL_STATE_INVALID_AUTH_SPEC, Messages.getString("SQLError.54")
	).put(
		SQL_STATE_SYNTAX_ERROR, Messages.getString("SQLError.55")
	).put(
		SQL_STATE_BASE_TABLE_OR_VIEW_NOT_FOUND,
		Messages.getString("SQLError.56")
	).put(
		SQL_STATE_BASE_TABLE_OR_VIEW_ALREADY_EXISTS,
		Messages.getString("SQLError.57")
	).put(
		SQL_STATE_BASE_TABLE_NOT_FOUND,
		Messages.getString("SQLError.58")
	).put(
		SQL_STATE_INDEX_ALREADY_EXISTS,
		Messages.getString("SQLError.59")
	).put(
		SQL_STATE_INDEX_NOT_FOUND, Messages.getString("SQLError.60")
	).put(
		SQL_STATE_COLUMN_ALREADY_EXISTS,
		Messages.getString("SQLError.61")
	).put(
		SQL_STATE_COLUMN_NOT_FOUND, Messages.getString("SQLError.62")
	).put(
		SQL_STATE_NO_DEFAULT_FOR_COLUMN,
		Messages.getString("SQLError.63")
	).put(
		SQL_STATE_GENERAL_ERROR, Messages.getString("SQLError.64")
	).put(
		SQL_STATE_MEMORY_ALLOCATION_FAILURE,
		Messages.getString("SQLError.65")
	).put(
		SQL_STATE_INVALID_COLUMN_NUMBER,
		Messages.getString("SQLError.66")
	).put(
		SQL_STATE_ILLEGAL_ARGUMENT, Messages.getString("SQLError.67")
	).put(
		SQL_STATE_DRIVER_NOT_CAPABLE, Messages.getString("SQLError.68")
	).put(
		SQL_STATE_TIMEOUT_EXPIRED, Messages.getString("SQLError.69")
	).build();

	private static final ImmutableMap<
		Integer, String
	> MYSQL_TO_SQL99_STATE = ImmutableMap.<Integer, String>builder().put(
		ErrorNumbers.ER_SELECT_REDUCED, SQL_STATE_WARNING
	).put(
		ErrorNumbers.ER_WARN_TOO_FEW_RECORDS, SQL_STATE_WARNING
	).put(
		ErrorNumbers.ER_WARN_TOO_MANY_RECORDS, SQL_STATE_WARNING
	).put(
		ErrorNumbers.ER_WARN_DATA_TRUNCATED, SQL_STATE_WARNING
	).put(
		ErrorNumbers.ER_WARN_NULL_TO_NOTNULL, SQL_STATE_NULL_VALUE_NOT_ALLOWED
	).put(
		ErrorNumbers.ER_WARN_DATA_OUT_OF_RANGE, SQL_STATE_NUMERIC_VALUE_OUT_OF_RANGE
	).put(
		ErrorNumbers.ER_SP_UNINIT_VAR, SQL_STATE_WARNING
	).put(
		ErrorNumbers.ER_SIGNAL_WARN, SQL_STATE_WARNING
	).put(
		ErrorNumbers.ER_SP_FETCH_NO_DATA, SQL_STATE_NO_DATA
	).put(
		ErrorNumbers.ER_SIGNAL_NOT_FOUND, SQL_STATE_NO_DATA
	).put(
		ErrorNumbers.ER_CON_COUNT_ERROR, SQL_STATE_CONNECTION_REJECTED
	).put(
		ErrorNumbers.ER_NOT_SUPPORTED_AUTH_MODE, SQL_STATE_CONNECTION_REJECTED
	).put(
		ErrorNumbers.ER_BAD_HOST_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_HANDSHAKE_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_UNKNOWN_COM_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_SERVER_SHUTDOWN, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_FORCING_CLOSE, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_IPSOCK_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_ABORTING_CONNECTION, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_PACKET_TOO_LARGE, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_READ_ERROR_FROM_PIPE, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_FCNTL_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_PACKETS_OUT_OF_ORDER, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_UNCOMPRESS_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_READ_ERROR, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_READ_INTERRUPTED, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_ERROR_ON_WRITE, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NET_WRITE_INTERRUPTED, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_NEW_ABORTING_CONNECTION, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_MASTER_NET_READ, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_MASTER_NET_WRITE, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_CONNECT_TO_MASTER, SQL_STATE_COMMUNICATION_LINK_FAILURE
	).put(
		ErrorNumbers.ER_SP_BADSELECT, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_SP_BADSTATEMENT, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_SP_SUBSELECT_NYI, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_STMT_NOT_ALLOWED_IN_SF_OR_TRG, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_SP_NO_RETSET, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_ALTER_OPERATION_NOT_SUPPORTED, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_ALTER_OPERATION_NOT_SUPPORTED_REASON, SQL_STATE_FEATURE_NOT_SUPPORTED
	).put(
		ErrorNumbers.ER_DBACCESS_DENIED_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_BAD_DB_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_FIELD_WITH_GROUP, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_GROUP_FIELD, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_SUM_SELECT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_LONG_IDENT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_DUP_KEYNAME, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_FIELD_SPEC, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_PARSE_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_EMPTY_QUERY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NONUNIQ_TABLE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_INVALID_DEFAULT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_MULTIPLE_PRI_KEY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_MANY_KEYS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_MANY_KEY_PARTS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_LONG_KEY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_KEY_COLUMN_DOES_NOT_EXITS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_BLOB_USED_AS_KEY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_BIG_FIELDLENGTH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_AUTO_KEY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_FIELD_TERMINATORS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_BLOBS_AND_NO_TERMINATED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_CANT_REMOVE_ALL_FIELDS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_CANT_DROP_FIELD_OR_KEY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_BLOB_CANT_HAVE_DEFAULT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_DB_NAME, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_TABLE_NAME, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_BIG_SELECT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_UNKNOWN_PROCEDURE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_PARAMCOUNT_TO_PROCEDURE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_FIELD_SPECIFIED_TWICE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_UNSUPPORTED_EXTENSION, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TABLE_MUST_HAVE_COLUMNS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_UNKNOWN_CHARACTER_SET, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_BIG_ROWSIZE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_OUTER_JOIN, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NULL_COLUMN_IN_INDEX, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_PASSWORD_ANONYMOUS_USER, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_PASSWORD_NOT_ALLOWED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_PASSWORD_NO_MATCH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_REGEXP_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_MIX_OF_GROUP_FUNC_AND_FIELDS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NONEXISTING_GRANT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TABLEACCESS_DENIED_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_COLUMNACCESS_DENIED_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_ILLEGAL_GRANT_FOR_TABLE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_GRANT_WRONG_HOST_OR_USER, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NONEXISTING_TABLE_GRANT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NOT_ALLOWED_COMMAND, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SYNTAX_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_LONG_STRING, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TABLE_CANT_HANDLE_BLOB, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TABLE_CANT_HANDLE_AUTO_INCREMENT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_COLUMN_NAME, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_KEY_COLUMN, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_BLOB_KEY_WITHOUT_LENGTH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_PRIMARY_CANT_HAVE_NULL, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_MANY_ROWS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_REQUIRES_PRIMARY_KEY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_KEY_DOES_NOT_EXITS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_CHECK_NO_SUCH_TABLE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_CHECK_NOT_IMPLEMENTED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_MANY_USER_CONNECTIONS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NO_PERMISSION_TO_CREATE_USER, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_USER_LIMIT_REACHED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SPECIFIC_ACCESS_DENIED_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NO_DEFAULT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_VALUE_FOR_VAR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_TYPE_FOR_VAR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_CANT_USE_OPTION_HERE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NOT_SUPPORTED_YET, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_FK_DEF, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_DERIVED_MUST_HAVE_ALIAS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TABLENAME_NOT_ALLOWED_HERE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SPATIAL_CANT_HAVE_NULL, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_COLLATION_CHARSET_MISMATCH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_NAME_FOR_INDEX, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_NAME_FOR_CATALOG, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_UNKNOWN_STORAGE_ENGINE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_ALREADY_EXISTS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_DOES_NOT_EXIST, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_LILABEL_MISMATCH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_LABEL_REDEFINE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_LABEL_MISMATCH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_BADRETURN, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_UPDATE_LOG_DEPRECATED_IGNORED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_UPDATE_LOG_DEPRECATED_TRANSLATED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_WRONG_NO_OF_ARGS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_COND_MISMATCH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_NORETURN, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_BAD_CURSOR_QUERY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_BAD_CURSOR_SELECT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_CURSOR_MISMATCH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_UNDECLARED_VAR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_DUP_PARAM, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_DUP_VAR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_DUP_COND, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_DUP_CURS, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_VARCOND_AFTER_CURSHNDLR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_CURSOR_AFTER_HANDLER, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_PROCACCESS_DENIED_ERROR, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NONEXISTING_PROC_GRANT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_BAD_SQLSTATE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_CANT_CREATE_USER_WITH_GRANT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_DUP_HANDLER, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_NOT_VAR_ARG, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_BIG_SCALE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_BIG_PRECISION, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_M_BIGGER_THAN_D, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_LONG_BODY, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TOO_BIG_DISPLAYWIDTH, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_BAD_VAR_SHADOW, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_WRONG_NAME, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SP_NO_AGGREGATE, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_MAX_PREPARED_STMT_COUNT_REACHED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_NON_GROUPING_FIELD_USED, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_PARAMCOUNT_TO_NATIVE_FCT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_PARAMETERS_TO_NATIVE_FCT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_PARAMETERS_TO_STORED_FCT, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_FUNC_INEXISTENT_NAME_COLLISION, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_DUP_SIGNAL_SET, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_SPATIAL_MUST_HAVE_GEOM_COL, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_TRUNCATE_ILLEGAL_FK, SQL_STATE_SYNTAX_ERROR
	).put(
		ErrorNumbers.ER_WRONG_NUMBER_OF_COLUMNS_IN_SELECT, SQL_STATE_CARDINALITY_VIOLATION
	).put(
		ErrorNumbers.ER_OPERAND_COLUMNS, SQL_STATE_CARDINALITY_VIOLATION
	).put(
		ErrorNumbers.ER_SUBQUERY_NO_1_ROW, SQL_STATE_CARDINALITY_VIOLATION
	).put(
		ErrorNumbers.ER_DUP_KEY, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_BAD_NULL_ERROR, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_NON_UNIQ_ERROR, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_DUP_ENTRY, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_DUP_UNIQUE, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_NO_REFERENCED_ROW, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_ROW_IS_REFERENCED, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_ROW_IS_REFERENCED_2, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_NO_REFERENCED_ROW_2, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_FOREIGN_DUPLICATE_KEY, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_DUP_ENTRY_WITH_KEY_NAME, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_FOREIGN_DUPLICATE_KEY_WITH_CHILD_INFO, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_FOREIGN_DUPLICATE_KEY_WITHOUT_CHILD_INFO, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_DUP_UNKNOWN_IN_INDEX, SQL_STATE_INTEGRITY_CONSTRAINT_VIOLATION
	).put(
		ErrorNumbers.ER_DATA_TOO_LONG, SQL_STATE_STRING_DATA_RIGHT_TRUNCATION
	).put(
		ErrorNumbers.ER_CANT_CREATE_GEOMETRY_OBJECT, SQL_STATE_NUMERIC_VALUE_OUT_OF_RANGE
	).put(
		ErrorNumbers.ER_DATA_OUT_OF_RANGE, SQL_STATE_NUMERIC_VALUE_OUT_OF_RANGE
	).put(
		ErrorNumbers.ER_TRUNCATED_WRONG_VALUE, SQL_STATE_INVALID_DATETIME_FORMAT
	).put(
		ErrorNumbers.ER_ILLEGAL_VALUE_FOR_TYPE, SQL_STATE_INVALID_DATETIME_FORMAT
	).put(
		ErrorNumbers.ER_DATETIME_FUNCTION_OVERFLOW, SQL_STATE_DATETIME_FIELD_OVERFLOW
	).put(
		ErrorNumbers.ER_DIVISION_BY_ZERO, SQL_STATE_DIVISION_BY_ZERO
	).put(
		ErrorNumbers.ER_SP_CURSOR_ALREADY_OPEN, SQL_STATE_INVALID_CURSOR_STATE
	).put(
		ErrorNumbers.ER_SP_CURSOR_NOT_OPEN, SQL_STATE_INVALID_CURSOR_STATE
	).put(
		ErrorNumbers.ER_CANT_DO_THIS_DURING_AN_TRANSACTION, SQL_STATE_INVALID_TRANSACTION_STATE
	).put(
		ErrorNumbers.ER_READ_ONLY_TRANSACTION, SQL_STATE_INVALID_TRANSACTION_STATE
	).put(
		ErrorNumbers.ER_ACCESS_DENIED_ERROR, SQL_STATE_INVALID_AUTH_SPEC
	).put(
		ErrorNumbers.ER_ACCESS_DENIED_NO_PASSWORD_ERROR, SQL_STATE_INVALID_AUTH_SPEC
	).put(
		ErrorNumbers.ER_ACCESS_DENIED_CHANGE_USER_ERROR, SQL_STATE_INVALID_AUTH_SPEC
	).put(
		ErrorNumbers.ER_DA_INVALID_CONDITION_NUMBER, SQL_STATE_INVALID_CONDITION_NUMBER
	).put(
		ErrorNumbers.ER_NO_DB_ERROR, SQL_STATE_INVALID_CATALOG_NAME
	).put(
		ErrorNumbers.ER_RESIGNAL_WITHOUT_ACTIVE_HANDLER, SQL_STATE_RESIGNAL_WHEN_HANDLER_NOT_ACTIVE
	).put(
		ErrorNumbers.ER_GET_STACKED_DA_WITHOUT_ACTIVE_HANDLER, SQL_STATE_STACKED_DIAGNOSTICS_ACCESSED_WITHOUT_ACTIVE_HANDLER
	).put(
		ErrorNumbers.ER_SP_CASE_NOT_FOUND, SQL_STATE_CASE_NOT_FOUND_FOR_CASE_STATEMENT
	).put(
		ErrorNumbers.ER_WRONG_VALUE_COUNT, SQL_STATE_INSERT_VALUE_LIST_NO_MATCH_COL_LIST
	).put(
		ErrorNumbers.ER_WRONG_VALUE_COUNT_ON_ROW, SQL_STATE_INSERT_VALUE_LIST_NO_MATCH_COL_LIST
	).put(
		ErrorNumbers.ER_INVALID_USE_OF_NULL, SQL_STATE_NULL_VALUE_NOT_ALLOWED
	).put(
		ErrorNumbers.ER_INVALID_ARGUMENT_FOR_LOGARITHM, SQL_STATE_INVALID_LOGARITHM_ARGUMENT
	).put(
		ErrorNumbers.ER_CANT_CHANGE_TX_ISOLATION, SQL_STATE_ACTIVE_SQL_TRANSACTION
	).put(
		ErrorNumbers.ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION, SQL_STATE_READ_ONLY_SQL_TRANSACTION
	).put(
		ErrorNumbers.ER_SP_NO_RECURSIVE_CREATE, SQL_STATE_SRE_PROHIBITED_SQL_STATEMENT_ATTEMPTED
	).put(
		ErrorNumbers.ER_SP_NORETURNEND, SQL_STATE_SRE_FUNCTION_EXECUTED_NO_RETURN_STATEMENT
	).put(
		ErrorNumbers.ER_TABLE_EXISTS_ERROR, SQL_STATE_ER_TABLE_EXISTS_ERROR
	).put(
		ErrorNumbers.ER_BAD_TABLE_ERROR, SQL_STATE_BASE_TABLE_OR_VIEW_NOT_FOUND
	).put(
		ErrorNumbers.ER_UNKNOWN_TABLE, SQL_STATE_BASE_TABLE_OR_VIEW_NOT_FOUND
	).put(
		ErrorNumbers.ER_NO_SUCH_TABLE, SQL_STATE_BASE_TABLE_OR_VIEW_NOT_FOUND
	).put(
		ErrorNumbers.ER_NO_SUCH_INDEX, SQL_STATE_ER_NO_SUCH_INDEX
	).put(
		ErrorNumbers.ER_DUP_FIELDNAME, SQL_STATE_ER_DUP_FIELDNAME
	).put(
		ErrorNumbers.ER_BAD_FIELD_ERROR, SQL_STATE_ER_BAD_FIELD_ERROR
	).put(
		ErrorNumbers.ER_ILLEGAL_REFERENCE, SQL_STATE_ER_BAD_FIELD_ERROR
	).put(
		ErrorNumbers.ER_QUERY_INTERRUPTED, SQL_STATE_ER_QUERY_INTERRUPTED
	).put(
		ErrorNumbers.ER_OUTOFMEMORY, SQL_STATE_MEMORY_ALLOCATION_ERROR
	).put(
		ErrorNumbers.ER_OUT_OF_SORTMEMORY, SQL_STATE_MEMORY_ALLOCATION_ERROR
	).put(
		ErrorNumbers.ER_XA_RBROLLBACK, SQL_STATE_XA_RBROLLBACK
	).put(
		ErrorNumbers.ER_XA_RBDEADLOCK, SQL_STATE_XA_RBDEADLOCK
	).put(
		ErrorNumbers.ER_XA_RBTIMEOUT, SQL_STATE_XA_RBTIMEOUT
	).put(
		ErrorNumbers.ER_XA_RMERR, SQL_STATE_XA_RMERR
	).put(
		ErrorNumbers.ER_XAER_NOTA, SQL_STATE_XAER_NOTA
	).put(
		ErrorNumbers.ER_XAER_INVAL, SQL_STATE_XAER_INVAL
	).put(
		ErrorNumbers.ER_XAER_RMFAIL, SQL_STATE_XAER_RMFAIL
	).put(
		ErrorNumbers.ER_XAER_DUPID, SQL_STATE_XAER_DUPID
	).put(
		ErrorNumbers.ER_XAER_OUTSIDE, SQL_STATE_XAER_OUTSIDE
	).put(
		ErrorNumbers.ER_LOCK_WAIT_TIMEOUT, SQL_STATE_ROLLBACK_SERIALIZATION_FAILURE
	).put(
		ErrorNumbers.ER_LOCK_DEADLOCK, SQL_STATE_ROLLBACK_SERIALIZATION_FAILURE
	).build();
}
